...
interface Props {
  onClose: () => void;
  onSave: (data: Omit<Website, 'id' | 'createdAt'>) => Promise<void>;
}

export default function AddWebsiteModal({ onClose, onSave }: Props) {
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          ...
          <div style={{ display: 'flex', gap: '8px', paddingTop: '16px', borderTop: '1px solid #dedede' }}>
            ...
            <button
              type="submit"
              disabled={loading}
              style={{ flex: 1, fontFamily: IT, fontWeight: 500, fontSize: '13px', padding: '12px 0', borderRadius: '8px', border: 'none', background: '#222222', color: '#ffffff', cursor: loading ? 'not-allowed' : 'pointer', opacity: loading ? 0.7 : 1 }}
            >
