...
interface Props { onClose: () => void; onSave: (data: SaveData) => Promise<void>; }
...
export default function AddModal({ onClose, onSave }: Props) {
  ...
  const [inputsNeeded, setInputs] = useState('');
  ...
  const readFile = useCallback((file: File) => {
    ...
    r.onload = e => { setImage(e.target?.result as string); setImageFile(file); };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      await onSave({ prompt: prompt.trim(), imageFile, image, tags, model: model || null, inputsNeeded: inputsNeeded.trim() || null });
      onClose();
      ...
      >
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          <div style={fieldStyle}>
            ...
            />
            <span style={{ fontFamily: IT, fontSize: '11px', color: '#b8b8b8', textAlign: 'right' }}>{prompt.length} / {MAX_CHARS}</span>
          </div>
          ...
          <div style={fieldStyle}>
            ...
            <input type="text" value={inputsNeeded} onChange={e => setInputs(e.target.value)} placeholder="e.g. Your photo, product image…" style={inputStyle}
          ...
          <div style={{ display: 'flex', gap: '8px', paddingTop: '8px' }}>
            ...
            <button type="submit" disabled={!prompt.trim() || saving} style={{ flex: 1, fontFamily: IT, fontWeight: 500, fontSize: '13px', padding: '9px 0', borderRadius: '8px', border: 'none', background: prompt.trim() ? '#222222' : '#b8b8b8', color: '#ffffff', cursor: prompt.trim() ? 'pointer' : 'not-allowed', transition: 'background 0.15s' }}>
